    ...
    >
      <el-form :model="editedUser" label-width="80px">
        ...
        <el-form-item label="请求ID">
          <el-input v-model="editedUser.joinId" disabled></el-input>
        </el-form-item>
        <el-form-item label="用户ID">
          <el-input v-model="editedUser.memberId" disabled></el-input>
        </el-form-item>
        <el-form-item label="用户名">
          <el-input v-model="editedUser.memberName" disabled></el-input>
        </el-form-item>
        <el-form-item label="用户昵称">
          <el-input v-model="editedUser.memberNickname" disabled></el-input>
        </el-form-item>
        <!-- 其他需要编辑的字段 -->
      </el-form>
      <el-button type="primary" @click="handleSaveEdit">同意</el-button>
      <el-button type="primary" @click="handleExit">拒绝</el-button>
    </el-dialog>
  </div>
</template>
  ...
  methods: {
    async handleExit() {
      try {
        ...
        this.editedUser.joinStatus = 3;
        const response = await axios.post('http://localhost:8080/updateJoin', this.editedUser, {
    ...
    async fetchUserInfoAndUserList() {
      try {
        ...
        // 发送 GET 请求获取用户列表数据
        const responseList = await axios.get('http://localhost:8080/listJoin/'+responseInfo.data.info.userPermission.userId, {
          withCredentials: true,
        });
